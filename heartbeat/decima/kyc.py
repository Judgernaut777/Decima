...
def _urllib_transport(url: str, headers: dict, body: str):
    ...
    raising, so `verify` decides the status. A transport-level failure (DNS, timeout,
    TLS) raises — `verify` maps that to KYCUnreachable. Never used by the offline oracle
