...
def _urllib_transport(url: str, headers: dict, body: str):
    ...
    raising, so `request_ride` decides SUCCEEDED/FAILED/UNKNOWN. A transport-level failure
    (DNS, timeout, TLS) raises — `request_ride` maps that to UNKNOWN. Never used by the
