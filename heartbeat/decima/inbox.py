...
class ApprovalInbox:
    ...
    def submit(self, agent_cell, cap_id, args, *, description=None, provenance=None):
        ...
        return {"queued": self.enqueue(agent_cell, cap_id, args,
                                       description=description, provenance=provenance)}

    def enqueue(self, agent_cell, cap_id, args, *, description=None, provenance=None) -> str:
        ...
        item_id = content_id({"inbox_item": cap_id, "args": args, "nonce": nonce,
                              "agent": agent_cell.id, "at": self.k.weft.head})
        assert_content(self.k.weft, principal, item_id, ITEM, {
            "capability": cap_id,
    ...
    def approve(self, item_id, agent_cell=None) -> dict:
        ...
        res = self.k.invoke(agent, cap_id, args, nonce=nonce)
        ...
        assert_content(self.k.weft, self.k.human.id, did, DECISION, {
            "item": c.id, "decision": "approved", "approver": self.k.human.id,
    ...
    def deny(self, item_id, reason="") -> str:
        ...
        assert_content(self.k.weft, self.k.human.id, did, DECISION, {
            "item": c.id, "decision": "denied", "approver": self.k.human.id,
            "capability": c.content.get("capability"), "reason": reason, "ran": False,
        })
        assert_edge(self.k.weft, self.k.human.id, did, "decides", c.id)
        return did
