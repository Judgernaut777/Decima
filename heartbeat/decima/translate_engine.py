...
def _urllib_transport(url: str, headers: dict, body: str):
    ...
    raising, so `translate` decides success vs. definite error. A transport-level
    failure (DNS, timeout, TLS) raises — `translate` maps that to
    `TranslateEngineError` (unreachable). Never used by the offline oracle (tests
