...
def _urllib_transport(url: str, headers: dict, body: str):
    ...
    raising, so `payout` decides SUCCEEDED/FAILED/UNKNOWN. A transport-level failure
    (DNS, timeout, TLS) raises — `payout` maps that to UNKNOWN. Never used by the offline
    oracle (tests inject a fake transport)."""
    import urllib.request
    import urllib.error
    if not url.startswith("https://"):                        # HTTPS-only — money never over cleartext
        raise executor.ExecError("payout: refusing a non-HTTPS endpoint")
