...
def _urllib_transport(url: str, headers: dict, body: str):
    ...
    raising, so `file_claim` decides SUCCEEDED/FAILED/UNKNOWN. A transport-level failure
    (DNS, timeout, TLS) raises — `file_claim` maps that to UNKNOWN. Never used by the
