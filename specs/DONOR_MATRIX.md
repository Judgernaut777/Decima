...
| Baidu Unlimited-OCR | Wrap as an OCR worker; port the inference interface | MIT (code, © 2026 Baidu); Hugging Face model weights license separate — verify | visual/browser worker, Weave ingestion, multimodal index, Studio | High-fidelity document/screenshot/image OCR as an attenuated worker; its output enters as **untrusted, provenance-stamped text** (`instruction_eligible=false`) feeding RAGFlow-style ingestion and memory | Model-weights license + vendor provenance; GPU/wheel dependency; OCR output is untrusted data and must never be obeyed as instruction |
| Google Fuchsia / FuchsiAware | Inspiration; reimplement the capability-model lessons | Apache-2.0 (the VS Code extension); Fuchsia OS itself is BSD-style — verify | capability model, Shell (component/source navigation), Morta | Fuchsia's component-URL + manifest + capability-routing model — the canonical real-world object-capability OS — as design validation for Decima capabilities; FuchsiAware's URL→manifest linking as a Shell UX for inspecting capability wiring | FuchsiAware is only a navigation extension, not the OS; the real value is Fuchsia's far larger component/capability design (a separate study) — don't mistake the tool for the architecture |
| cx-core (CX Linux) | Inspiration (study-only) | BUSL-1.1 — commercial-use restricted; converts to Apache-2.0 six years after each release | Shell, terminal/session executor | The shape of an AI-native terminal core: a natural-language→Linux-ops daemon (`cx-daemon`) over a GPU terminal — reference only | BUSL competing-use restriction → study-only until cleared/converted; it is a **WezTerm fork** (use upstream WezTerm MIT, not cx's relicensed delta); single-vendor |
| cx-distro (CX Linux) | Inspiration (study-only) | BSL-1.1 — commercial-use restricted | Linux appliance/distro build, Shell | The recipe for building an AI-shell Linux distro: live-build / debootstrap / squashfs / xorriso ISO pipeline, apt repo + supply-chain security, automated install — directly informs Decima's "minimal Linux appliance that boots into the Decima shell" form factor | BSL license → study-only; project maturity; Debian-packaging specifics are vendor-shaped |
| ubuntu-zombie | Reimplement concepts; port selected code | MIT (© 2026 Eric Mourant / Japer Technology) | Morta, executor, Shell, Linux appliance | The human-in-the-loop **propose → approve → watch → audit-logged** pattern for an AI operating a real OS; safe-default hardening (key-only SSH, root-disabled, opt-in tailnet); an AI sysadmin as an *attenuated principal* whose every action is gated and logged — maps cleanly onto Morta gates + Weft provenance + a sandboxed root worker | A root-capable AI is the highest-blast-radius design: it must be an attenuated, sandboxed principal with Morta gates on every irreversible op and full receipts — **never ambient root**; Ubuntu-specific |
| Walturn AI-OS survey | Inspiration (landscape reference; no code) | Editorial content — cite, don't copy | vision/positioning, Shell, model router | Competitive landscape (Fuchsia, Azure Sphere, Watson, Ubuntu AI, Tesla, "Steve") and an AI-OS capability checklist: unified shared-memory layer for cross-agent collaboration, agent marketplace/discovery, multi-agent orchestration, self-healing autonomy, real-time/edge/multimodal | Marketing-level survey, not technical depth; ideas need grounding in Decima primitives, not adoption as-is |

## Architectural consequences

1. No donor owns a canonical Decima type. Adapters translate into Cells, Events, Capabilities, Receipts, and Attestations.
2. AGPL, FSL, BUSL/BSL, noncommercial, and unclear-license repositories are study-only unless a later legal review explicitly clears a boundary.
