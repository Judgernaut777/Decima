...
def build() -> dict:
    ...
    def record(ev):
        events.append({
            "id": ev.id, "verb": ev.verb, "lamport": ev.lamport,
            "authorized": ev.authorized, "body": ev.body,
        })

    cid_type = model.define_type(weft, author, "note")
    record(model.assert_content(weft, author, "note:1", "note",
                                {"text": "milestone two", "n": 1}))
    record(model.assert_content(weft, author, "note:1", "note",
                                {"text": "attested note", "n": 2}))

    # Capability grant + downhill attenuation (same construction as v1).
    parent_cap = capability_content(name="pay", effect="pay", target="*",
                                    caveats={"budget": 100}, grantee=author,
                                    granter=author)
    parent_id = hashing.content_id({"cap": "pay", "v": 1})
    record(weft.append(author, ASSERT,
                       {"cell": parent_id, "type": "capability",
                        "content": parent_cap}))
    child_cap = attenuate(parent_cap, {"budget": 40}, parent_id,
                          grantee=author, granter=author)
    child_id = hashing.content_id({"cap": "pay", "v": 1, "att": 1})
    record(weft.append(author, ASSERT,
                       {"cell": child_id, "type": "capability",
                        "content": child_cap}))
    ...
    record(weft.append(author, INVOKE,
                       {"cap": child_id, "args": {"amount": 10, "cost": 10},
                        "nonce": "decima-ext-nonce-1"},
                       authorized=child_id))
    record(weft.append(author, INVOKE,
                       {"cap": child_id, "args": {"amount": 5, "cost": 5},
    ...
    record(weft.append(author, ATTEST,
                       {"target_cell": "note:1", "claim": "verified by author"}))
    record(weft.append(attester, ATTEST,
                       {"target_cell": "note:1", "claim": "witnessed"}))
    record(weft.append(author, ATTEST,
                       {"target_cell": child_id, "claim": "cap review ok"}))

    # (a) The exact stored payload TEXT bytes, straight from the table.
    stored = [{"seq": seq, "payload": payload}
              for seq, payload in weft.db.execute(
    ...
    attestations = {
        ...
        for cid in sorted(first.cells) if first.cells[cid].attestations
    }

    return {
        "profile": "decima:v0.1 — extended vectors (SQLite persistence, warm start, INVOKE/ATTEST fold)",
        ...
        "invocations": [
            {"event": i.event, "by": i.by, "cap": i.cap, "args": i.args}
...
def main() -> None:
    ...
    print(f"  events: {vectors['event_count']} "
    ...
    print(f"  state_root: {vectors['state_root'][:24]}… "
