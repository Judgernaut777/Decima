...
## Instance 1 — kernel · sandboxed-principal substrate  (clone `~/decima-work`)
...
```text
You are the worker kernel/isolation instance for Decima, in ~/decima-work. Read
docs/BACKLOG.md (brief SB1), specs/MORTA_CAPABILITIES.md, and heartbeat/checks/README.md first.

Task SB1 — branch work/sb1-sandbox — the no-ambient-authority linchpin:
...
Bootstrap: scripts/kickoff.sh ~/decima-work work/sb1-sandbox
...
## Instance 2 — worker · networked sync at scale  (worktree `~/decima-work-gx`)

`git worktree add ~/decima-work-gx work/gx1-gossip`.
...
```text
You are a worker sync-at-scale instance for Decima, in a dedicated worktree. Read docs/BACKLOG.md
(brief GX1), specs/SYNC.md, heartbeat/decima/sync.py (SY2), and heartbeat/checks/README.md first.

Task GX1 — branch work/gx1-gossip — generalize SY2 to N peers, efficiently:
...
Bootstrap: scripts/kickoff.sh ~/decima-work-gx work/gx1-gossip
...
## Instance 3 — worker · voice contract slice  (worktree `~/decima-work-vox`)

`git worktree add ~/decima-work-vox work/vox1-voice`.
...
```text
You are a worker voice instance for Decima, in a dedicated worktree. Read docs/BACKLOG.md (brief
VOX1), specs/BROWSER_WORKER.md (the stub-engine + untrusted-data pattern), and
heartbeat/checks/README.md first.

Task VOX1 — branch work/vox1-voice — the voice contract with a deterministic stub:
...
Bootstrap: scripts/kickoff.sh ~/decima-work-vox work/vox1-voice
