...
## Instance 1 — worker · detection-as-code (security beachhead)  (worktree `~/decima-work-det`)

`git worktree add ~/decima-work-det work/det1-detection`.
...
```text
You are a worker detection-engineering instance for Decima, in a dedicated worktree. Read
docs/BACKLOG.md (brief DET1), specs/CAPABILITY_MAP.md Part C, and heartbeat/checks/README.md first.

Task DET1 — branch work/det1-detection — detection-as-code on Decima's own primitives:
...
Bootstrap: scripts/kickoff.sh ~/decima-work-det work/det1-detection
...
## Instance 2 — worker · capability inspector + constellation  (worktree `~/decima-work-ins`)

`git worktree add ~/decima-work-ins work/ins1-inspector`.
...
```text
You are a worker inspector/constellation instance for Decima, in a dedicated worktree. Read
docs/BACKLOG.md (brief INS1), specs/CAPABILITY_MAP.md (A2 + D1), specs/MORTA_CAPABILITIES.md,
and heartbeat/checks/README.md first.

Task INS1 — branch work/ins1-inspector — exact projections over the Weave/Weft:
...
Bootstrap: scripts/kickoff.sh ~/decima-work-ins work/ins1-inspector
...
## Instance 3 — worker · agent shorthand  (worktree `~/decima-work-sh`)

`git worktree add ~/decima-work-sh work/sh1-shorthand`.
...
```text
You are a worker shorthand instance for Decima, in a dedicated worktree. Read docs/BACKLOG.md
(brief SH1), specs/CAPABILITY_MAP.md (D2), and heartbeat/checks/README.md first.

Task SH1 — branch work/sh1-shorthand — an auditable, reversible token-compression transport:
...
Bootstrap: scripts/kickoff.sh ~/decima-work-sh work/sh1-shorthand
