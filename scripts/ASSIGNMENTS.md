...
## Instance 1 — worker · Secrets broker  (worktree `~/decima-work-cred`)

`git worktree add ~/decima-work-cred work/cred1-secrets`.
...
```text
You are a worker secrets-broker instance for Decima, in a dedicated worktree. Read docs/BACKLOG.md
...
Task CRED1 — branch work/cred1-secrets — the credential layer powerbox defers:
...
Bootstrap: scripts/kickoff.sh ~/decima-work-cred work/cred1-secrets
...
## Instance 2 — worker · Self-hosted / private inference  (worktree `~/decima-work-inf`)

`git worktree add ~/decima-work-inf work/inf1-inference`.
...
```text
You are a worker private-inference instance for Decima, in a dedicated worktree. Read docs/BACKLOG.md
...
Task INF1 — branch work/inf1-inference — rent-a-GPU / self-host, data never leaves:
...
Bootstrap: scripts/kickoff.sh ~/decima-work-inf work/inf1-inference
...
## Instance 3 — kernel · Live governance gate  (clone `~/decima-work`)
...
```text
You are the worker kernel instance for Decima, in ~/decima-work. Read docs/BACKLOG.md (brief LOOP1),
...
Task LOOP1 — branch work/loop1-live-governance — make governance live:
...
Bootstrap: scripts/kickoff.sh ~/decima-work work/loop1-live-governance
