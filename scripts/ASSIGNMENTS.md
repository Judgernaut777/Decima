...
## Instance 1 — kernel · Live disposition loop  (clone `~/decima-work`)
...
```text
You are the worker kernel instance for Decima, in ~/decima-work. Read docs/BACKLOG.md (brief
INTAKE1), decima/disposition.py (DISP1 — dispose()), the existing kernel.ingest_observation, and
heartbeat/checks/README.md first.

Task INTAKE1 — branch work/intake1-live-intake — make the disposition router live:
...
Bootstrap: scripts/kickoff.sh ~/decima-work work/intake1-live-intake
...
## Instance 2 — worker · Blue-team triage / SIEM  (worktree `~/decima-work-triage`)

`git worktree add ~/decima-work-triage work/triage1-siem`.
...
```text
You are a worker blue-team instance for Decima, in a dedicated worktree. Read docs/BACKLOG.md (brief
TRIAGE1), specs/CAPABILITY_MAP.md Part C, decima/detection.py (DET1 emits `finding` cells), and
heartbeat/checks/README.md first.

Task TRIAGE1 — branch work/triage1-siem — turn detections into incidents:
...
Bootstrap: scripts/kickoff.sh ~/decima-work-triage work/triage1-siem
...
## Instance 3 — worker · Trading on the payments rail  (worktree `~/decima-work-trade`)

`git worktree add ~/decima-work-trade work/trade1-trading`.
...
```text
You are a worker trading instance for Decima, in a dedicated worktree. Read docs/BACKLOG.md (brief
...
Task TRADE1 — branch work/trade1-trading — trade stocks on the rail:
...
Bootstrap: scripts/kickoff.sh ~/decima-work-trade work/trade1-trading
