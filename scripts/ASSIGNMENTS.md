...
1. `kernel.py` / `weft.py` / `weave.py` / `executor.py` are **owned by the worker /
...
## Instance 1 — worker · kernel  (clone: `~/decima-work`)
...
```text
You are the worker kernel instance for Decima, in ~/decima-work. Read docs/BACKLOG.md
and heartbeat/checks/README.md first.

Task D3 — branch work/d3-org-policy:
...
Bootstrap: scripts/kickoff.sh ~/decima-work work/d3-org-policy
...
## Instance 2 — review worker · CLI worker  (clone: `~/decima-review`)
...
```text
You are the review worker instance for Decima, in ~/decima-review. Read docs/BACKLOG.md and
heartbeat/checks/README.md first.

Task D1 — branch review/d1-cli-worker:
...
Bootstrap: scripts/kickoff.sh ~/decima-review review/d1-cli-worker
...
## Instance 3 — worker · sessions  (worktree: `~/decima-work-router`, or a fresh one)

Reuse the existing router worktree, or make a fresh one:
`git worktree add ~/decima-work-sessions work/d2-sessions`.
...
```text
You are a worker sessions instance for Decima, in a dedicated worktree (NOT the main
~/decima-work tree). Read docs/BACKLOG.md and heartbeat/checks/README.md first.

Task D2 — branch work/d2-sessions:
...
Bootstrap: scripts/kickoff.sh <your-worktree-dir> work/d2-sessions
