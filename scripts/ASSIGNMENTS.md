...
## Instance 1 — worker · merge layer (the hard one)  (clone: `~/decima-work`)
...
```text
You are the worker merge-layer instance for Decima, in ~/decima-work. Read
docs/BACKLOG.md (brief M1), specs/MERGE_SEMANTICS.md, and heartbeat/checks/README.md first.

Task M1 — branch work/m1-merge-impl — implement the FIRST increment of the merge layer:
...
Bootstrap: scripts/kickoff.sh ~/decima-work work/m1-merge-impl
...
## Instance 2 — review worker · memory maturation  (clone: `~/decima-review`)
...
```text
You are the review worker memory instance for Decima, in ~/decima-review. Read docs/BACKLOG.md
(brief B3) and heartbeat/checks/README.md first.

Task B3 — branch review/b3-memory-maturation:
...
Bootstrap: scripts/kickoff.sh ~/decima-review review/b3-memory-maturation
...
## Instance 3 — worker · router engines  (worktree, e.g. `~/decima-work-router`)

Reuse the router worktree or make a fresh one:
`git worktree add ~/decima-work-engines work/c2-router-engines`.
...
```text
You are a worker router-engines instance for Decima, in a dedicated worktree (NOT the main
~/decima-work tree). Read docs/BACKLOG.md (brief C2) and heartbeat/checks/README.md first.

Task C2 — branch work/c2-router-engines:
...
Bootstrap: scripts/kickoff.sh <your-worktree-dir> work/c2-router-engines
...
## Instance 4 — review worker · sync spec  (worktree: `~/decima-review-sync`)

`git worktree add ~/decima-review-sync review/s1-sync-spec` (from the codex clone).

**Task:** S1. **Owns:** `specs/SYNC.md` (new). **Must not touch:** anything else.

```text
You are a review worker spec instance for Decima, in a dedicated worktree. Read docs/BACKLOG.md
(brief S1), specs/WEFT_PROTOCOL.md §9, specs/FOLD_AND_LIFECYCLE.md §9, and
specs/MERGE_SEMANTICS.md first.

Task S1 — branch review/s1-sync-spec — write specs/SYNC.md (design only, NO code):
...
Bootstrap: scripts/kickoff.sh ~/decima-review-sync review/s1-sync-spec
...
## Instance 5 — review worker · snapshots spec  (worktree: `~/decima-review-snap`)

`git worktree add ~/decima-review-snap review/s2-snapshots-spec` (from the codex clone).
...
Task S2 — branch review/s2-snapshots-spec — write specs/SNAPSHOTS.md (design only, NO code):
...
Bootstrap: scripts/kickoff.sh ~/decima-review-snap review/s2-snapshots-spec
