...
Per-instance briefs for the **current cycle (4) — worker instances only.** Tasks/lanes
...
## Instance 1 — worker · merge classes (core)  (clone: `~/decima-work`)
...
```text
You are the worker merge instance for Decima, in ~/decima-work. Read docs/BACKLOG.md
(brief M2), specs/MERGE_SEMANTICS.md, and heartbeat/checks/README.md first.

Task M2 — branch work/m2-merge-classes — build the remaining merge classes on M1's
...
Bootstrap: scripts/kickoff.sh ~/decima-work work/m2-merge-classes
...
## Instance 2 — worker · snapshots  (worktree, e.g. `~/decima-work-snap`)

`git worktree add ~/decima-work-snap work/sn1-snapshots`.
...
```text
You are a worker snapshots instance for Decima, in a dedicated worktree (NOT the main
~/decima-work tree). Read docs/BACKLOG.md (brief SN1), specs/SNAPSHOTS.md, and
heartbeat/checks/README.md first.

Task SN1 — branch work/sn1-snapshots — realize SNAPSHOTS.md's first increment:
...
Bootstrap: scripts/kickoff.sh ~/decima-work-snap work/sn1-snapshots
...
## Instance 3 — worker · sync convergence  (optional; worktree `~/decima-work-sync`)

`git worktree add ~/decima-work-sync work/sy1-sync-sim`.
...
```text
You are a worker sync-sim instance for Decima, in a dedicated worktree. Read docs/BACKLOG.md
(brief SY1), specs/SYNC.md, and heartbeat/checks/README.md first.

Task SY1 — branch work/sy1-sync-sim — exercise SYNC.md in the reference (no network):
...
Bootstrap: scripts/kickoff.sh ~/decima-work-sync work/sy1-sync-sim
...
## Notes
- **worker only this cycle** (by request). All three lanes push over the `~/decima-work` SSH
