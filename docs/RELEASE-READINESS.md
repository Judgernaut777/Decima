...
| Projections rebuild from the Weft | **verified** | `tests/projections/test_rebuild_equals_incremental.py`, `tests/api/test_projection_rebuild_preserves_state.py` | host | in the full gate (626) | — |
| Plans / jobs survive restart | **verified** | `tests/e2e/test_crash_recovery.py`, `tests/runtime/test_supervisor.py` | host | in the full gate (626) | — |
| Sensitive effects require bound approval | **verified** | `tests/e2e/test_approval_gating.py` (deny → approve-once → consumed → reuse-fails); approve route is `REAUTH` | host | in the full gate (626); audit §7 | — |
| Revoked authority stops future use | **verified** | `tests/e2e/test_revocation.py` (cascade; receipts preserved) | host | in the full gate (626) | — |
...
| Kernel / protocol frozen | **verified** | `git diff --quiet` `heartbeat/` vs the frozen heartbeat baseline (tree `30bb20dc`, reachable via `29bfe9a`), `decima/kernel/` + `protocol/` vs `29bfe9a` → byte-identical | host | audit §1 | — |
| Conformance / golden fixtures | **verified** | `pytest tests/kernel/test_conformance.py` (canonical-bytes / event-id / fold-state-root / tamper-detect) | host | in the full gate (626) | — |
...
- **License metadata (I-1)**: `pyproject.toml` declares `license = "Proprietary"` /
  ...
  This tension with the project's open-reference framing is **decision-pending** as of this
  assessment: the owner must choose whether to adopt an explicit open-source license or keep
  the proprietary declaration. Until that decision is recorded, the `pyproject.toml` metadata
  is left unchanged.
...
1. Documentation matches reality — test/spec counts corrected (**done**: 626/25 + 13 specs/9
...
3. Re-run the full gate on the exact tag commit; confirm still **626/25**, `heartbeat/` /
