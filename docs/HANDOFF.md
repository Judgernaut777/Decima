...
Extract impl sources from the workflow journal → write module+check to repo → `cd heartbeat && python3 smoke.py` (must end `heartbeat: alive. ✓` exit 0, all checks green) → append a house-style Cycle NN entry to `docs/BACKLOG.md` (before the `**Tooling — ✅**` line) → `git add -A && commit` → `git pull --rebase origin main` → `git push origin HEAD:main`. Concurrent sessions can advance main between calls — always fetch/rebase, re-Read before editing.
