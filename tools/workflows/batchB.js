export const meta = {
  name: 'decima-batchB-fossilize',
  description: 'Batch B — correctness debts the Rust port must not fossilize: rotation-aware event verification (the one core lane), forge-real discovery wiring, real parallel effect execution. All Fable 5; each adversarially mutation-reviewed (Fable 5).',
  phases: [
...
This lane EDITS weft.py (module_path = heartbeat/decima/weft.py, FULL updated) + optionally kernel.py (extra_file). Register your OWN hermetic effect if needed (e.g. 'rot_probe'), never 'echo'.`
  ...
  {
    name: 'forgereal', module: 'forge.py', nn: '464', model: 'fable', effort: 'high', edits: true, core: false, extra: '',
    title: 'FORGE-REAL DISCOVERY — route forge-if-missing through the real candidate→reckoner→promotion pipeline, not the honest stub',
    seams: 'forge.py (synthesize_manifest at 70, _stub_handler at 94, forge at 111 — TODAY forge returns an honest STUB: stub=True/forged=True, no outward effect, no real test/scan/promotion), discovery.py (discover(..., forge=...) — the caller that reaches forge when nothing is found/installed; study how it invokes forge), candidate.py (author_candidate at 143 — intent→codegen→a born-quarantined candidate), reckoner.py (evaluate at 243 — sandboxed test + property + hostile-input eval), promotion.py (promote at 143 — attested promotion + register_version; PromotionBlocked), checks that prove the forge-real loop (grep FORGE / self-extension).',
    spec: `
PURPOSE: P3’s forge-real loop (intent→codegen→sandboxed test→scan→attested promotion→versioning) exists as candidate.py/reckoner.py/promotion.py, but discovery’s forge-if-missing STILL reaches forge.py’s honest STUB (stub=True handlers that do nothing) — two parallel forge entry points, and the discovery-triggered one is fake. Route it through the REAL pipeline so a caller that reaches self-extension gets a tested, scanned, attested capability, not a stub.

HARDEN decima/forge.py (edit ONLY forge.py; if discovery.py needs one wiring line to reach the real path, return it via extra_file_path/extra_file_source, disclosed): make forge (the discovery-triggered path) an ADAPTER over candidate.author_candidate → reckoner.evaluate → promotion.promote — a forged capability is BORN QUARANTINED, EVALUATED (sandboxed test + property + hostile input), and PROMOTED through the attested gate (or refused, fail closed) — instead of returning a decorative stub. Keep synthesize_manifest (the honest description) if still useful, but the RESULT of a discovery-forge must be a real promoted (or refused) capability, never a silent stub passed off as real.
CONSTRAINTS: born quarantined (default-deny); promotion is the only authority path; a candidate that FAILS evaluation is NOT promoted (PromotionBlocked, fail closed); keep every existing check green (including whatever currently exercises the stub path — adjust minimally + disclose if the stub semantics legitimately change).

CHECK checks/464_forgereal.py proves, offline + deterministically (injected deterministic codegen):
  (a) DISCOVERY-FORGE YIELDS A REAL CAPABILITY (load-bearing): forging a missing capability now runs candidate→reckoner→promotion — the result is a PROMOTED, non-stub capability (born quarantined, evaluated, attested), and it actually runs its real behavior; assert it is NOT stub=True/forged-stub.
  (b) A FAILING CANDIDATE IS REFUSED: a candidate that fails evaluation is NOT promoted (PromotionBlocked / stays quarantined) — fail closed, no silent stub fallback.
  (c) NO REGRESSION: existing forge/discovery checks stay green.
  Mutation: revert forge to return the stub (skip the real pipeline) → (a) goes RED (the forged capability is a stub, not a promoted real one). State the load_bearing_line.
This lane EDITS forge.py: module_path = heartbeat/decima/forge.py (FULL updated). Register your OWN hermetic effect if needed (e.g. 'forge_probe'), never 'echo'.`
  },
  {
    name: 'parfx', module: 'concurrency.py', nn: '466', model: 'fable', effort: 'high', edits: true, core: false, extra: '',
    title: 'REAL PARALLEL EFFECTS — worker effects actually OVERLAP (only the Weft commit serializes), exactly-once + clean fold preserved',
    seams: 'concurrency.py (Cycle 55: run_concurrent/race/claim_loop — TODAY worker threads only CLAIM job ids off a queue and every EFFECT runs serially in the single Weft-owner thread; the _APPEND_LOCK serializes the commit; the honest single-writer-queue fallback), jobs.py (run → kernel.invoke: the invoke appends INVOKE then runs the effect handler; the single-use lease is exactly-once), kernel.py (invoke; lease_uses), executor.py (the effect handler seam — effects are pure-ish functions run under the executor), checks/436_concurrency.py (the existing check — MUST stay green).',
    spec: `
PURPOSE: Cycle 55’s concurrency lane is honest that parallelism is NOMINAL — worker threads only move job ids between queues; every effect runs serially in the owner thread. Make effect execution ACTUALLY overlap: the effect HANDLER work runs concurrently across workers, while ONLY the Weft mutation (the INVOKE append + status transition) is serialized under the lock. Prove real overlap while keeping the two laws: exactly-once (the single-use lease) and a clean fold (serialized commit, no seq/parent corruption), with a deterministic (non-flaky) proof of concurrency.

HARDEN decima/concurrency.py (edit ONLY concurrency.py): split each job into (1) the effect-handler execution that CAN overlap across workers, and (2) the serialized Weft-commit critical section (the append + status transition) under the existing lock — so two workers’ effects are genuinely in-flight simultaneously while their commits serialize. Keep exactly-once (a worker racing an already-fired job is denied by the exhausted lease) and a clean fold (Kernel(db, fresh=False) re-verifies + folds with no WeftError). NO wall-clock/thread-id/float in recorded content; the fired-SET stays identical to a serial reactor.tick.
DETERMINISM OF THE PROOF: prove overlap without timing flakiness — e.g. effects rendezvous on a shared threading.Barrier(parties=K) so the suite completes ONLY if K effects are genuinely in-flight AT ONCE (were execution serial, the barrier would never trip and the check would fail loud on a bounded timeout). The barrier is the deterministic witness of real concurrency.

CHECK checks/466_parfx.py proves, offline + deterministically:
  (a) EFFECTS REALLY OVERLAP (load-bearing): K independent jobs whose effects rendezvous on a Barrier(parties=K) ALL complete — provable only if their effect handlers ran CONCURRENTLY (a serial runner would deadlock/timeout); assert all K done AND the barrier tripped (max concurrent in-flight == K).
  (b) EXACTLY-ONCE UNDER CONTENTION: many workers racing ONE single-use job → exactly 1 fires (lease_uses == 1), the rest denied by the exhausted lease.
  (c) CLEAN FOLD + SAME FIRED-SET: after a parallel run, Kernel(db, fresh=False) folds with no WeftError; the fired-set equals a serial reactor.tick’s.
  (d) INTS / no wall-clock in recorded Cells; existing 436 stays green.
  Mutation: force effects back to serial (run the handler INSIDE the commit lock / drop the overlap) → (a) goes RED (the Barrier never trips; the check times out / fails). State the load_bearing_line.
This lane EDITS concurrency.py: module_path = heartbeat/decima/concurrency.py (FULL updated). Register your OWN hermetic effect if needed (e.g. 'parfx_probe'), never 'echo'.`
  },
]
