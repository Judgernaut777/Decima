...
//
// The security-critical screens (Q&A cited answer, Plans, Approval inbox, Workspace,
// Self-extension) are screenshotted at both viewports into docs/release-evidence/visual/ for
// the reviewer to inspect. Kept deliberately to a small set of PNGs.
//
// Those PNGs are EVIDENCE, so they must be REPRODUCIBLE: a second run against an unchanged
// tree has to leave them byte-identical, or a real UI regression is indistinguishable from
// host-speed noise. The transient toast stack is cleared immediately before every capture for
// exactly that reason (see `clearToasts` for the measured evidence and the rejected
// alternatives) — with it cleared, every one of the ten files reproduces byte-for-byte, so
// any modification to them is a genuine UI-change signal and should be reviewed as one.
//
// CAVEAT on that guarantee, stated rather than hidden: two assertions below deliberately
// tolerate either of two rendered states (the plan pill /ACTIVE|COMPLETED/i and the organ
// /PROMOTED|QUARANTINED/). Byte-identity therefore holds per host; a host that raced those
// transitions differently would legitimately capture the other pill. Tighten those two first
// if byte-identity is ever asserted in CI across heterogeneous runners.
...
// Remove every live toast before a capture, and return how many were removed.
//
// WHY THIS EXISTS, AND WHAT IT REFUSES. The toast stack (`#toasts`, `position: fixed`) is the
// ONE thing on these screens that is not a function of the Weft. `toast()` in
// `decima/shell/frontend/js/app.js` drops each node on a WALL-CLOCK `setTimeout(..., 4000)`,
// so the number of toasts still painted when a screenshot fires is a function of how fast the
// host executed the preceding steps — nothing else. That made the release evidence
// unreproducible: two back-to-back full-suite runs on an unchanged tree rewrote
// `plans-desktop.png`, `plans-mobile.png` and `qa-mobile.png`, and every differing pixel lay
// inside the toast boxes (e.g. a run that still had "Workspace run created" alive vs. one that
// did not). Clearing the host makes the captures a pure function of the seeded Cells.
//
// It also removes a REAL defect in the evidence, not just noise: an unpatched `qa-mobile.png`
// had "Termination sent to the Approval inbox" and "Approved once" painted over the
// "OPERATOR QUESTION" and "MODEL ANSWER — GENERATED BY DETERMINISTIC-OFFLINE" zone labels —
// transient chrome obscuring the exact trust boundary the screenshot exists to prove.
//
// What it does NOT do: it does not mask, wait out, or otherwise paper over a rendered
// difference. Masking was rejected on evidence — `mask:` paints the element's bounding box,
// and that box grows and shrinks with the toast count, so the mask itself would still vary.
// Waiting (`toHaveCount(0)`) would cost up to 4s per capture and put the wall clock back into
// the test. Everything outside `#toasts` is already deterministic: Cell ids (a fixed identity
// seed, no clock in signed content), layout, fonts (no `@font-face`, remote fonts CSP-refused)
// and animations (there are none) reproduce byte-for-byte.
//
// The pending removal timer guards on `node.parentNode`, so orphaning the nodes is safe.
async function clearToasts(page, ctxLabel) {
  ...
  // A missing host means this guard has silently stopped working and the evidence has gone
  // back to varying with host speed. Fail loudly rather than keep shipping PNGs that claim
  // to be reproducible and are not.
  expect(
    removed,
    `${ctxLabel}: #toasts host is gone — the screenshot determinism guard no longer works`
    ...
    for (const vp of VIEWPORTS) {
      ...
      for (const [id, title] of SCREENS) {
        ...
        if (shotScreens.has(id)) {
          // Transient toasts are the only non-Weft-derived pixels on these screens and they
          // expire on a wall clock: clear them so the evidence is byte-reproducible and so no
          // toast can sit on top of a trust-zone label. See `clearToasts` for the evidence.
          await clearToasts(page, where);
